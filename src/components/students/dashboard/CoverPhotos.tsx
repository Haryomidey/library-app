...
function CoverPhotos({ subjects, justViewed }: CoverPhotosProps) {
  const recentlyViewed =
    justViewed?.length > 1 ? justViewed?.slice(0, 1)[0] : justViewed[0];
  const recentlyUploaded =
    subjects?.length > 1 ? subjects?.slice(0, 1)[0] : subjects[0];
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-10 mb-4">
      {recentlyViewed ? (
        <div
          className="relative rounded-xl bg-[#FAFAFA] cursor-pointer"
          onClick={() => handleRouting(recentlyViewed)}
        > 
          <img
          ...
          <div className="absolute text-black bottom-0 flex flex-col p-6 rounded-lg space-y-3">
            <span className="font-semibold text-xl bg-[white] px-4 py-1 rounded-md shadow-sm">
              {recentlyViewed?.subject_name}
            </span>
            <span className="font-light bg-[white] px-3 py-1 rounded-md w-[fit-content] shadow-sm">
              {recentlyViewed?.teacher_name}
            </span>
          </div>
        </div>
      ) : (
      ...
      {recentlyUploaded ? (
        <div
          className="relative rounded-xl bg-[#FAFAFA] cursor-pointer"
          onClick={() => handleRouting(recentlyUploaded)}
        >
          <img
            src={
              recentlyUploaded?.cover ? recentlyUploaded?.cover : DefaultImage
            }
            className="w-full object-cover h-72 rounded-lg"
          ...
          <div className="absolute text-black bottom-0 flex flex-col p-6 rounded-lg space-y-3">
            <span className="font-semibold text-xl bg-[white] px-4 py-1 rounded-md shadow-sm">
              {recentlyUploaded?.subject_name}
            </span>
            <span className="font-light bg-[white] px-3 py-1 rounded-md w-[fit-content] shadow-sm">
              {recentlyUploaded?.teacher_name}
            </span>
          </div>
        </div>
      ) : (
